...
From our analysis of simulated influenza- and coronavirus-like sequences, we found that each method produced consistent embeddings of genetic sequences for two distinct pathogens, 55 years of evolution, and a wide range of practical method parameters.
...
From these results, we conclude that tree-free dimensionality reduction methods provide valuable biological insights through easily interpretable visualizations of genetic relationships and the ability to account for genetic variation that phylogenetic methods cannot including indels, reassortment, and recombination.

% Limitations of methods and analysis
Ability to capture ``broad'' genetic groups but not ``fine'' groups
PCA only useful without missing data, then imputation of missing data required
t-SNE and UMAP limited in ability to maintain intermediate genetic structure

Lack of biological model is strength and weakness
Not surprising that distances reflect genetic relationships, since this is the essence of phylogenetics
However, we show how well tree-free methods can capture phylogenetic groups and groups from biologically informed models like TreeKnit based on distances alone
Not meant to replace ARGs and other biological methods but to supplement these methods or provide a initial quick investigation of the data akin to iterating on subsampling approaches with ML trees prior to full Bayesian tree inference
Other dimensionality methods may work even better, but we had to choose a subset for practical reasons
Other human pathogenic viruses with different transmission modes or genome architectures could be relevant like HIV, Zika, RSV, etc.
Runtime performance of each method depends on input data size
Analysis used a consistent, fixed input size throughout, but embeddings and their optimal parameters may vary with input size
More work required to scale distance calculations and embeddings to higher sample sizes
Limited number of recombinant lineages to compare

% Future directions and use of methods
Provide optimal settings for each method per pathogen.
Open source tools to apply to any pathogens where automated genetic group labels are desired
Easy integration with existing genomic epidemiology workflows to provide automated genetic group labels and low-dimensional visualizations of high-dimensional data without requiring a tree.
Could expand method to use amino acid distances instead of nucleotide distances, reducing the resolution of clusters but increasing their biological relevance
We anticipate that these methods could be readily adopted to jointly analyze multiple influenza gene segments or lineages and to identify genetic groups in bacterial and eukaryotic pathogens that cannot be studied by phylogenetic methods.

\section*{Conclusion}

We show that simple dimensionality reduction methods operating on pairwise genomic differences can capture biologically-relevant clusters of phylogenetic clades, reassortment events, and patterns of recombining lineages.
