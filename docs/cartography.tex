...
Therefore, we focused our reassortment analysis on HA and NA sequences, sampling 1,643 viruses collected between January 2016 and October 2018 with both HA and NA sequences.
...
We expected that clusters from HA-only embeddings would reflect little information about reassortment, as the embeddings lack any information about the NA gene.
Accordingly, we expected that the VI distances for clusters from HA/NA embeddings would improve on the baseline distances calculated with the HA-only clusters.

With the exception of PCA, all embedding methods produced more accurate clusters from the HA/NA alignments than the HA-only alignments (Fig.~\ref{fig:seasonal-influenza-h3n2-ha-na-2016-2018-embeddings}).
...
In contrast, the PCA clusters from HA/NA alignments were approximately 1.8 times farther from the known reassortment events than the HA-only clusters.
Although the first principal component in the HA/NA embedding explained 44\% of variation compared to 27\% in the HA-only embedding, the explained variation had no apparent relationship to the genetic diversity of the samples.
Inspection of the combined HA/NA alignments revealed gap characters at the start and end of many NA sequences.
Since PCA is the only method that counts the ``N'' character as a separate state, the PCA embedding most likely reflects these gap characters in the first component.
