...
Simulated genomes collected from the same time period tended to map closer in embedding space, indicating the maintenance of ``local'' genetic structure in the embeddings (Fig.~\ref{fig:simulated-populations-representative-embeddings}).
...
These qualitative results matched our expectations based on the optimization of method parameters to maximize linear relationships between genetic and Euclidean distances.
...
We selected the distance value per method that minimized the distance between HDBSCAN clusters and clade assignments as measured by the normalized variation of information (VI) metric \cite{meilua2003comparing} (see Methods).
...
Like UMAP, the t-SNE clusters captured broader clades including the A4 clade that UMAP clustered with 3c2.A while also failing to distinguish among A1b and its subclades.
Interestingly, t-SNE clusters also included 3 clusters not found in any other embeddings.
The largest of these (cluster 3) corresponded to a subclade of A2 that was previously associated with a reassortment to a different neuraminidase background \cite{Potter2019}.
Although cluster 7 contained only 22 samples, it descended from a subclade of A1a and its samples carried substitutions at known epitope sites (HA1:140M and HA1:193S).
Cluster 8 descended from clade A1 with 29 samples and contained 14 samples with a substitution at another known epitope site (HA1:135K).
Although these clades never reached high global frequencies, they represented biologically relevant samples that would have been of interest to contemporary researchers.
...
MDS clusters capture more of the known clades, but they also collect all clades descended from A1 into the same cluster and suffer from more unassigned samples than the other embeddings.
The PCA embedding only produced 3 clusters with the lowest accuracy (normalized VI=0.18).
