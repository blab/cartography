...
Unlike the early H3N2 HA dataset, the late dataset contained fewer clades (\nameref{S5_Fig_late_flu_embeddings_by_clade}) and a greater genetic distance between samples in clades (\nameref{S4_Fig_flu_within_between_group_distances}).
...
Embeddings with both genes also produced more clusters than the HA-only embeddings with one additional cluster in PCA (\nameref{S9_Fig_flu_ha_na_pca_embeddings}), 11 in MDS (\nameref{S10_Fig_flu_ha_na_mds_embeddings}), five in t-SNE (\nameref{S11_Fig_flu_ha_na_tsne_embeddings}), and one in UMAP (\nameref{S12_Fig_flu_ha_na_umap_embeddings}).
