...
\caption{{\bf The phylogeny of influenza H3N2 viruses (top) shows the evolutionary relationships among viruses including clades, or viruses that share the same mutations and descend from the same common ancestor.}
...
Embeddings with PCA (upper left), MDS (upper right), t-SNE (lower left), and UMAP (lower right) consistently exhibit linear relationships for pairs of samples that differ by no more than 10 nucleotides.}
...
Embeddings with both genes also produced more clusters than the HA-only embeddings with two more in PCA (\nameref{S7_Fig_flu_ha_na_pca_embeddings}), nine more in MDS (\nameref{S8_Fig_flu_ha_na_mds_embeddings}), four more in t-SNE (\nameref{S9_Fig_flu_ha_na_tsne_embeddings}), and two more in UMAP (\nameref{S10_Fig_flu_ha_na_umap_embeddings}).
...
We identified recombinant lineages that always mapped closer to their parental lineages and recombinant lineages that always mapped closer to at least one of the parental lineages.

Only 5 of the 10 recombinant lineages had enough samples in both parental and recombinant lineages to calculate average pairwise distances (XD, XE, XG, XBB, and XBL).
