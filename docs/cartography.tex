...
As expected, the clusters for each method generally corresponded to larger phylogenetic clades (Fig~\ref{fig:seasonal-influenza-h3n2-ha-clusters}).
Clusters from t-SNE and UMAP embeddings most accurately captured expert clade assignments (normalized VI=0.07) with 11 and 6 clusters, respectively.
The 7 MDS clusters were nearly twice as far from expert clades as t-SNE and UMAP clusters (normalized VI=0.12).
The PCA embedding only produced 3 clusters with the lowest accuracy (normalized VI=0.17).
...
Similarly, MDS's ability to accurately represent genetic distance did not correspond to high-resolution clusters.
...
The tighter grouping of samples in t-SNE and UMAP embeddings enabled HBSCAN to detect more accurate clusters.
The 11 clusters from the t-SNE embedding most completely captured clade assignments including clusters for clades A1, A2, A3, A4, and 3c3.A.
As expected, HDBSCAN clustered all smaller subclades of A1b into a single cluster (0).
