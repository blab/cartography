...
def get_hamming_distances(genomes):
    ...
    for i in range(len(genomes)):
        ...
        for j in range(i + 1, len(genomes)):
            ...
            if(mismatches.sum() - (mismatches & valid_bases[i] & valid_bases[j]).sum() == 0):
                hamming_distances.append((mismatches & valid_bases[i] & valid_bases[j]).sum())
            else:
                hamming_distances.append((mismatches & valid_bases[i] & valid_bases[j]).sum() / float(mismatches.sum() - (mismatches & valid_bases[i] & valid_bases[j]).sum()))
