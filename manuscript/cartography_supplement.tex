...
\begin{table}
\caption{{\bf Number of clusters (\textit{n_clusters}) identified by HDBSCAN, transitions between clusters in the phylogeny, and excess transitions indicating non-monophyletic groups per pathogen dataset and embedding method.}
  This table reports the list of specific clusters per dataset and method (\textit{clusters}), the number of transitions between different cluster labels on the phylogeny (\textit{n_cluster_transitions}), and the list of observed transitions for pairs of clusters (\textit{transitions}).
  The number of excess transitions between clusters (\textit{n_extra_transitions}) reflects the number of times that we observed a transition from one source cluster to another beyond the one expected transition.
...
Distances include average pairwise comparisons between $A$ and $B$ (\textit{distance_A_B}), $A$ and $X$ (\textit{distance_A_X}), and $B$ and $X$ (\textit{distance_B_X}).
Additional columns indicate whether each recombinant lineage maps closer to both parental lineages (or at least one) than those parents map to each other (\textit{X_maps_closer_to_both_parentals} and \textit{X_maps_closer_to_any_parental}, respectively).
Records with values of ``True'' in the column \textit{X_maps_closer_to_both_parentals} represent the expected placement of the recombinant lineage between its two parental lineages.
