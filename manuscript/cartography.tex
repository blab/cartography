...
The optimal parameters were 2 components for PCA, 3 for MDS, perplexity of 100 and learning rate of 200 for t-SNE, and nearest neighbors of 50 and minimum distance of 0.1 for UMAP.
...
Clusters produced with the optimal distance threshold were generally monophyletic (Supplementary Table~S\ref{S_Table_monophyletic_clusters}), supported by cluster-specific mutations (Supplementary Table~S\ref{S_Table_mutations_per_cluster}), and corresponded to larger phylogenetic clades (Fig~\ref{fig:seasonal-influenza-h3n2-ha-2016-2018-clusters}).
Pairwise genetic distances between sequences in the same MDS, t-SNE, or UMAP clusters matched the distances between sequences within Nextstrain clades (Supplementary Fig.~S\ref{S_Fig_flu_within_between_group_distances}).
...
Clusters from all four methods captured broader phylogenetic clades (A1, A1b, A2, A3, 3c2.A, and 3c3.A) but failed to distinguish between A1b and its descendants.
...
As with the early dataset, clusters from the late dataset were largely monophyletic (Supplementary Table~S\ref{S_Table_monophyletic_clusters}), supported by cluster-specific mutations (Supplementary Table~S\ref{S_Table_mutations_per_cluster}), and corresponded to larger phylogenetic clades (Fig.~\ref{fig:seasonal-influenza-h3n2-ha-2018-2020-clusters} and Supplementary Fig.~S\ref{S_Fig_late_flu_embeddings_by_clade})
Pairwise genetic distances within clusters generally matched the diversity within Nextstrain clades (Supplementary Fig.~S\ref{S_Fig_flu_within_between_group_distances}).
Clusters from PCA (N=5), MDS (N=6), t-SNE (N=7), and UMAP (N=8) were similarly accurate, with normalized VI distances of 0.07, 0.06, 0.05, and 0.09, respectively (Fig.~\ref{fig:seasonal-influenza-h3n2-ha-2018-2020-clusters} and Supplementary Fig.~S\ref{S_Fig_late_flu_mds_embeddings}).
...
All embedding methods produced more accurate clusters from the HA/NA alignments than the HA-only alignments (Fig.~\ref{fig:seasonal-influenza-h3n2-ha-na-2016-2018-embeddings} and Supplementary Fig.~S\ref{S_Fig_full_ha_na_embeddings}).
HA/NA clusters from MDS reduced the distance to known reassortment events from a normalized VI value of 0.16 with HA only to 0.05.
Similarly, HA/NA clusters from t-SNE reduced the distance from 0.10 to 0.06 and reduced the distance for PCA clusters from 0.18 to 0.12.
Adding NA to HA only modestly improved UMAP clusters, reducing the distance to MCCs from 0.14 to 0.11.
Embeddings with both genes produced more clusters in PCA, MDS, and t-SNE than the HA-only embeddings with 3 additional clusters in PCA (Supplementary Fig.~S\ref{S_Fig_flu_ha_na_pca_embeddings}), 9 in MDS (Supplementary Fig.~S\ref{S_Fig_flu_ha_na_mds_embeddings}), 2 in t-SNE (Supplementary Fig.~S\ref{S_Fig_flu_ha_na_tsne_embeddings} and Supplementary Fig.~S\ref{S_Fig_flu_ha_na_tsne_mcc_counts}), and 1 in UMAP (Supplementary Fig.~S\ref{S_Fig_flu_ha_na_umap_embeddings}).
...
Both t-SNE and UMAP maintained intermediate degrees of linearity (Pearson's $R^{2}=0.57$ and $R^{2}=0.63$, respectively).
...
However, we found that the optimal thresholds were the same for both lineage definitions except for a lower threshold for t-SNE with Pango lineages (Supplementary Table~S\ref{S_Table_optimal_cluster_parameters}).
...
Clusters from all methods were generally supported by cluster-specific mutations including 2 of 3 PCA clusters (67\%), 10 of 15 MDS clusters (67\%), 11 of 15 t-SNE clusters (73\%) and 5 of 6 UMAP clusters (83\%) (Supplementary Table~S\ref{S_Table_mutations_per_cluster}).
The 15 clusters from t-SNE were closest to the 24 Nextstrain clades (normalized VI=0.06), while the other methods were 2 to 4 times farther away (Fig.~\ref{fig:sars-cov-2-2020-2022-clusters-vs-Nextstrain-clade})
Clusters from t-SNE also had the most similar within-group distances to Nextstrain clades (Supplementary Fig.~S\ref{S_Fig_sarscov2_within_between_group_distances}).

Clusters from all methods were farther from the 35 Pango lineages (Supplementary Fig.~S\ref{S_Fig_sarscov2_early_embeddings_by_cluster_vs_Nextclade_pango}).
The 19 clusters from t-SNE were the closest (normalized VI=0.13), while clusters from all other methods were at least two or three times farther from Pango lineages.
...
Only t-SNE clusters were fully monophyletic (Supplementary Table~S\ref{S_Table_monophyletic_clusters}).
PCA and t-SNE clusters were best supported by cluster-specific mutations with 6 of 7 clusters (86\%) and 31 of 41 clusters (76\%), respectively (Supplementary Table~S\ref{S_Table_mutations_per_cluster}).
Clusters from t-SNE had the lowest within-group distances (Supplementary Fig.~S\ref{S_Fig_sarscov2_within_between_group_distances}).
Compared to the 18 Nextstrain clades defined in this time period, the closest clusters were from t-SNE (normalized VI=0.04) and UMAP (normalized VI=0.09, Fig.~\ref{fig:sars-cov-2-2022-2023-clusters-vs-Nextstrain-clade}).
However, t-SNE produced 41 clusters, nearly three times more than UMAP's 15.
...
We observed similar absolute and relative distances to Nextstrain clades across methods at different sampling densities under an even geographic and temporal sampling scheme (Supplementary Fig.~S\ref{S_Fig_late_sarscov2_replication_of_cluster_accuracy}).
Even in the presence of geographic and genetic bias associated with randomly sampling the late SARS-CoV-2 data, t-SNE and UMAP produced the most accurate clusters.
...
All methods produced less accurate representations of the 137 Pango lineages (Supplementary Fig.~S\ref{S_Fig_sarscov2_late_embeddings_by_cluster_vs_Nextclade_pango}).
...
Of the 83 recombinant Pango lineages that also had a t-SNE cluster, 79 (95\%) mapped to a single t-SNE cluster (Supplementary Fig.~S\ref{S_Fig_sarscov2_late_embeddings_tsne_recombinant_counts}).
Of the 54 t-SNE clusters with recombinant Pango lineages, 42 (78\%) mapped to a single Pango lineage.
...
MDS embeddings most consistently placed recombinant lineages between the parental lineages with correct placement of 59 (89\%) lineages (e.g., Supplementary Fig.~S\ref{S_Fig_sarscov2_late_embeddings_mds_recombinant_examples}).
The t-SNE, PCA, and UMAP embeddings correctly placed 54 (82\%), 45 (68\%), and 40 (61\%) lineages, respectively.
...
Finally, we applied the \emph{umap-learn} Python package written by UMAP's authors, with options to set the number of ``nearest neighbors'' and the ``minimum distance'' \citep{lel2018umap}.
...
We applied TreeKnit to the rooted HA and NA trees with a gamma value of 2.0 and the `--better-MCCs` flag, as previously recommended for H3N2 analyses \citep{Barrat-Charlaix2022}.
...
Specifically, we applied \emph{augur traits} with TreeTime (version 0.10.1) \citep{Sagulenko2018,Huddleston2021} to infer cluster labels for internal nodes of the phylogeny for each pathogen dataset and embedding method.
...
The \emph{pathogen-embed} Python package, available at \href{https://pypi.org/project/pathogen-embed/}{https://pypi.org/project/pathogen-embed/}, provides command line utilities to calculate distance matrices (\emph{pathogen-distance}), calculate embeddings per method (\emph{pathogen-embed}), and apply hierarchical clustering to embeddings (\emph{pathogen-cluster}).
