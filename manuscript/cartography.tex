...
Embeddings from t-SNE appeared robust to variation in parameters, with a slight improvement in median MAE associated with perplexity of 100 and little benefit to any of the learning rate values (Supplementary Fig.~S\ref{S_Fig_simulated_flu_errors} C).
...
Clusters from PCA (N=6), MDS (N=6), t-SNE (N=5), and UMAP (N=8) were similarly accurate, with normalized VI distances of 0.09, 0.07, 0.08, and 0.06, respectively (Fig.~\ref{fig:seasonal-influenza-h3n2-ha-2018-2020-clusters} and Supplementary Fig.~S\ref{S_Fig_late_flu_mds_embeddings}).
...
Cluster accuracies were robust to changes in sampling density under the same even geographic and temporal sampling scheme, with PCA and MDS clusters producing the lowest median distance to Nextstrain clades (Supplementary Fig.~S\ref{S_Fig_late_flu_replication_of_cluster_accuracy} A).
...
Embeddings with both genes produced more clusters in PCA, MDS, and t-SNE than the HA-only embeddings with 1 additional cluster in PCA (Supplementary Fig.~S\ref{S_Fig_flu_ha_na_pca_embeddings}), 9 in MDS (Supplementary Fig.~S\ref{S_Fig_flu_ha_na_mds_embeddings}), 6 in t-SNE (Supplementary Fig.~S\ref{S_Fig_flu_ha_na_tsne_embeddings} and Supplementary Fig.~S\ref{S_Fig_flu_ha_na_tsne_mcc_counts}), and 0 in UMAP (Supplementary Fig.~S\ref{S_Fig_flu_ha_na_umap_embeddings}).
With the exception of PCA, all embeddings of HA/NA alignments produced distinct clusters for the known reassortment event within clade A2 \citep{Potter2019} as represented by MCCs 14 and 11.
...
Only clusters from t-SNE and UMAP represented completely monophyletic groups (Supplementary Table~S\ref{S_Table_monophyletic_clusters}).
These two methods also produced the most clusters supported by specific mutations (Supplementary Table~S\ref{S_Table_mutations_per_cluster}).
...
Clusters from all methods were farther from the 35 Pango lineages (Supplementary Fig.~S\ref{S_Fig_sarscov2_early_embeddings_by_cluster_vs_Nextclade_pango}), but t-SNE's clusters were the closest (normalized VI=0.12).
...
Only t-SNE and UMAP clusters were monophyletic (Supplementary Table~S\ref{S_Table_monophyletic_clusters}).
Only PCA and t-SNE had cluster-specific mutations for more than half their clusters (Supplementary Table~S\ref{S_Table_mutations_per_cluster}).
...
Of the 79 recombinant Pango lineages that also had a t-SNE cluster, 78 (99\%) mapped to a single t-SNE cluster (Supplementary Fig.~S\ref{S_Fig_sarscov2_late_embeddings_tsne_recombinant_counts}).
Of the 54 t-SNE clusters with recombinant Pango lineages, 45 (83\%) mapped to a single Pango lineage.
...
PCA performs poorly with missing or noisy data \citep{jolliffe_cadima_2016} and requires researchers to either ignore columns with missing values or impute the missing values prior to analysis, as previously shown for Zika virus \citep{metsky_2017}.
